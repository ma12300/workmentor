...
(function () {
  ...
  async function ensureChannel() {
    const rk = roomKey();
    if (!rk) { alert("먼저 소통방에 입장하세요."); return false; }
  ...
  window.SShare = {
    open: async function () {
      if (!roomKey()) { alert("소통방에 입장한 뒤 사용할 수 있습니다."); return; }
      showWrap(true);
      await ensureChannel();
      setStatus("준비됨 — [화면 공유 시작]을 누르면 내 화면을 상대에게 보여줍니다.");
    },
    start: startShare,
