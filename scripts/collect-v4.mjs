...
const GROUP_QUERIES = {
  ...
  fund:  "금융위원회 OR 관세청",
  biz:   "중소벤처기업부",
  money: "소상공인 정책자금 OR 창업 지원금 OR 수출바우처",
  guide: "부가세 신고 OR 종합소득세 OR 연말정산 OR 퇴직금",
};
...
function decode(s) {
  /* 순서 중요: CDATA 해제 → 문자참조 복원 → 그제서야 태그 제거 (역순이면 링크 코드가 날것으로 샘) */
  return String(s)
...
function parseRss(xml) {
  ...
  return items.map(b => {
    ...
    return {
      ...
      summary: "",   /* 구글뉴스 요약은 제목 반복 링크뿐이라 제외 */
      source,
    };
